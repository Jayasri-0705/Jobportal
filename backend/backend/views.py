...
from .serializers import ApplicationSerializer, JobsSerializers, RegisterSerializer

@api_view(['GET'])
def hello_api(request):
    return Response({"message":"Hello from Django framework API"})

@api_view(['POST'])
def register_user(request):
    serialiazer = RegisterSerializer(data=request.data)
    if serialiazer.is_valid():
        serialiazer.save() #saves users
